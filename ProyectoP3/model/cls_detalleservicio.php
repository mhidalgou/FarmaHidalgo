 ...
 *
 */
class cls_detalleservicio extends cls_conexion
{
  ...
  public function getAll(){
    $query = "SELECT id,nombre FROM detalle_servicio WHERE status = 0";
    $this->do_open();
  ...
  public function getAllLibres(){
    $query = "SELECT id,nombre FROM detalle_servicio WHERE status <> 2 and status <> 1";
    $this->do_open();
    $res = $this->do_query($query);
    if ($res->num_rows > 0) {
      while ($row = $res->fetch_object()) {//para parsear el resultado del select
